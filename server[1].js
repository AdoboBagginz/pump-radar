...
// Bot imports — wrapped so server starts even if bot deps missing
let initBot = async () => {};
let processScanData = async () => {};
let emergencyStop = async () => {};
let getBotStatus = () => ({ running: false, dryRun: true, solBalance: 0, positions: [], stats: {}, config: {} });
let getBotEvents = () => [];

try {
  const bot = await import("./bot.js");
  initBot        = bot.initBot;
  processScanData = bot.processScanData;
  emergencyStop  = bot.emergencyStop;
  getBotStatus   = bot.getBotStatus;
  getBotEvents   = bot.getBotEvents;
  console.log("[bot] modules loaded OK");
} catch(e) {
  console.error("[bot] failed to load bot modules:", e.message);
  console.error("[bot] server will run in scan-only mode");
}
